...
module tb_ctrlu;
  ...
  ctrlu instance1 (.opcode(opct), .operand(oprt), .zflag(zt), .cflag(ct), .input_data(inpdt), .alu_opcode(aluopct), .acc_load(acclt), .reg_read_addr(readaddrt), .reg_write_addr(writeaddrt), .reg_write_enable(writent), .pc_load(pclt), .pc_increment(pcinrt), .acc_src(accsrct), .out_enable(outent) );
      ...
      for (j = 0; j <=15; j = j + 1) begin
        for (k = 0; k <=1; k = k + 1) begin
          for (l = 0; l <=1; l = l + 1) begin
			...
			endcase
              if ({aluopct, acclt, readaddrt, writeaddrt, writent, pclt, pcinrt, accsrct, outent}!={exp_alu_opcode, exp_acc_load, exp_reg_read_addr, exp_reg_write_addr,exp_reg_write_enable, exp_pc_load, exp_pc_increment, exp_acc_src, exp_out_enable}) begin
